...
BOSS 直聘会逐个点开卡片读右侧完整 JD；实习僧、智联、牛客会并发抓同源详情页补全 JD；北森用站点自己的列表接口按标题合并完整 JD（新版与企业定制模板都支持）；Moka 新版模板卡片自带全文、经典模板自动逐岗打开详情页读取；飞书招聘抓同源岗位接口补全 JD。51job 和应届生因为详情页有反爬或滑动验证，只按卡片摘要打粗分。
...
| 北森 ATS `xxx.zhiye.com` | ✅ | ✅ 完整 JD | 企业校招官网（科大讯飞、合合信息等，兼容企业定制模板如中核 `/custom/campus`）；卡片没有链接，扫描时用站点自己的列表接口按标题合并完整 JD |
...
- **权限尽量小。** 固定权限是 `storage` + `activeTab` + `scripting`（`activeTab` 只在你主动点击时授权当前页，用于任意页面按需注入）；主机权限限定在十二个招聘站；LLM 接口域名走可选权限，保存配置时才按需申请。
